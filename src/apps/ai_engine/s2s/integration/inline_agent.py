...
# --- Constants ---
from core.settings.base import DEFAULT_REGION, DEFAULT_AGENT_MODEL_ID
...
class InlineAgentOrchestrator:
    """Orchestrates interactions with Amazon Bedrock Inline Agents for coach."""
    ...
    def _validate_config(self) -> None:
        if not os.path.isfile(self.config["schema_file"]):
            raise RuntimeError(
                f"API schema file not found: {self.config['schema_file']}"
            )
        if not os.getenv(LAMBDA_ARN_ENV):
            raise RuntimeError(
                f"{LAMBDA_ARN_ENV} environment variable must be set. Please run setup_booking_resources.sh and ensure .env is loaded."
            )
    ...
    def _get_lambda_arn() -> str:
        ...
        if not lambda_arn:
            raise RuntimeError(
                f"{LAMBDA_ARN_ENV} environment variable must be set."
            )
    ...
    def invoke(self, query: str) -> str:
        try:
            ...
            logger.info(f"Started at: {time_before_call}")
    ...
    def _process_response(self, agent_resp: Dict[str, Any]) -> str:
        ...
        try:
            ...
            logger.error(
                f"Caught exception while processing response from invokeAgent: {e}",
            )
    ...
    try:
        with _orchestrator_lock:
            if _orchestrator is not None:
                ...
                logger.info(
                    f"Releasing resources for session: {_orchestrator.session_id}"
                )
                _orchestrator = None
                logger.info("Inline agent resources cleaned up successfully")
    except Exception as e:
        logger.error(
            f"Error during inline agent cleanup: {str(e)}", exc_info=True
        )
        raise
